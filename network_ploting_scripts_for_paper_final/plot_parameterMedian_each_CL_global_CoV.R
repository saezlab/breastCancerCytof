# Summary script to produce Marco's data:
...
get_edge_data <- function(par_stats){
...
get_node_data <- function(par_stats){
...
nodes_CL_median = get_node_data(par_stats)
edges_CL_median = get_edge_data(par_stats)
...
for(i in seq_along(target_names)){
	nodes_CL_median$name[nodes_CL_median$name==target_names[[i]]] = names(target_names)[[i]]
	edges_CL_median$from = gsub(target_names[[i]],names(target_names)[[i]], edges_CL_median$from,fixed = T)
	edges_CL_median$to = gsub(target_names[[i]],names(target_names)[[i]], edges_CL_median$to,fixed = T)
	edges_CL_median$parameter = gsub(target_names[[i]],names(target_names)[[i]], edges_CL_median$parameter,fixed = T)
}
...
all_nodes <- nodes_CL_median %>% full_join(global_nodes,by = "name")
all_edges <- edges_CL_median %>% full_join(global_edges,by = c("parameter" ,"from","to"))
...
#	print(cl_name)
	...
	exportImage(filename = file.path(getwd(),"supp_info/results_for_april_2019/network_figs_v3",paste0(var_name,".pdf")),type = "PDF")
}
...
setEdgeColorMapping(var_name,
					#c(0, max(max(all_nodes$CoV_par,na.rm = T),max(all_edges$CoV_par,na.rm = T))),
...
exportImage(filename = file.path(getwd(),"supp_info/results_for_april_2019/network_figs_v3",paste0(var_name,"_orange.pdf")),type = "PDF")
